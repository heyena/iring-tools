  ...
  */
  initComponent: function () {
    ...
    this.treeLoader = new Ext.tree.TreeLoader({
      baseParams: { type: null, baseUri: null, related: null, text: null },
    ...
    this.treeLoader.on("beforeload", function (treeLoader, node) {
      ...
      else if (x.name == 'expand')
        x.scope.body.mask('Loading...', 'x-mask-loading');
      ...
      if (node.attributes != undefined) {
        ...
        if (node.attributes.properties != undefined) {
          ...
          if (node.attributes.properties.BaseURI)
            treeLoader.baseParams.baseUri = node.attributes.properties.BaseURI;
        }
        else if (node.attributes.property != undefined) {
          node.leaf = false;

          if (node.attributes.property.BaseURI)
            treeLoader.baseParams.baseUri = node.attributes.property.BaseURI;
