...
AdapterManager.NHibernateConfigWizard = Ext.extend(Ext.Container, {
	...
	constructor: function (config) {
		...
		var applyProperty = function (form) {
		...
		var changeConfigOracle = function (host, dbSchema, userName, password, serviceName) {
			host.setValue('');
			host.clearInvalid();
		...
		var changeConfig = function (ddbName, dbServer, dbInstance, dbSchema, userName, password) {
			ddbName.setValue('');
			dbName.clearInvalid();
			dbName.show();
		...
		var getDataTypeIndex = function (datatype) {
			for (var i = 0; i < dataTypes.length; i++) {
				if (dataTypes[i][1] == datatype)
					return dataTypes[i][0];
			}
		};
						...
						for (var k = 0; k < keyChildenNodes.length; k++) {
							...
							if (!keyNode.hidden) {
								...
								if (typeof keyNodeProf.dataType == 'string')
									tagProps.dataType = getDataTypeIndex(keyNodeProf.dataType);
								else
