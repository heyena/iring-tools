  ...
  constructor: function (config) {
    ...
    var setKeyProperty = function (editPane, node) {
      if (editPane && node) {
        ...
        var keyPropertyFormPanel = Ext.create('Ext.form.Panel',{
          name: 'keyProperty',
    ...
    var setDataProperty = function (editPane, node) {
      if (editPane && node) {
        ...
        var dataPropertyFormPanel = Ext.create('Ext.form.Panel', {
          name: 'dataProperty',
          ...
          tbar: new Ext.Toolbar({
            items: [{
              ...
              handler: function (f) {
                var form = dataPropertyFormPanel.getForm();
                setDataPropertyFields(form, node.data.properties);
              }
            }]
          })
        });
    ...
    var setDsConfigPane = function (editPane) {
      if (editPane) {
        ...
        if (!Ext.ModelManager.isRegistered('Providers.Model')) {
            Ext.define('Providers.Model', {
                extend: 'Ext.data.Model',
                fields: ['provider']
            });
        }
        var providersStore = Ext.create('Ext.data.JsonStore', {
        ...
        var dsConfigPane = Ext.create('Ext.form.Panel', {
          labelWidth: 140,
              ...
              handler: function (f) {
                ...
                dsConfigPane.getForm().submit({
                  ...
                  success: function (f, a) {
                    ...
                    var dbObjectsTreeStore = dataObjectsPane.items.items[0].items.items[0];
                    dbObjectsTreeStore.disable();
    ...
    var setAvailTables = function (dbObjectsTreeStore) {
      var availTableName = new Array();

      if (dbObjectsTreeStore.disabled) {
        ...
        var rootNode = dbObjectsTreeStore.getRootNode();
    ...
    var setSelectTables = function (dbObjectsTreeStore) {
    ...
    var setTablesSelectorPane = function (editPane) {
      ...
      if (editPane) {
        ...
        var availItems = setAvailTables(dbObjectsTreeStore);
        var selectItems = setSelectTables(dbObjectsTreeStore);

        var tablesSelectorPane = Ext.create('Ext.form.Panel', {
          frame: false,
          border: false,
          autoScroll: true,
          ...
          tbar: Ext.create('Ext.toolbar.Toolbar', {
            items: [{
              ...
              handler: function () {
                ...
                if (dbObjectsTreeStore.disabled) {
                  dbObjectsTreeStore.enable();
                }

                var treeStore = dbObjectsTreeStore.getLoader();
                ...
                treeStore.proxy.url = 'AdapterManager/DBObjects';
                if (dsConfigPane) {
              ...
              handler: function () {
                ...
                for (var j = 0; j < availTableName.length; j++)
                  for (var i = 0; i < rootNode.childNodes.length; i++) {
                    if (rootNode.childNodes[i].data.properties.tableName == availTableName[j]) {
                      found = true;
                      availTableName.splice(j, 1);
                      j--;
                      break;
                ...
                for (var i = 0; i < rootNode.childNodes.length; i++) {
                  var nodeText = rootNode.childNodes[i].data.properties.tableName;
                  selectTableNames.push([nodeText, nodeText]);
    ...
    var setRelations = function (editPane, node) {
      if (editPane && node) {
        ...
        var relationCreateFormPanel = Ext.create('Ext.form.Panel', {
          labelWidth: 155,
    ...
    var loadTree = function (rootNode) {
      ...
      for (var i = 0; i < rootNode.childNodes.length; i++) {
        ...
        dataObjectNode.data.properties.tableName = dataObjectNode.text;
        for (var ijk = 0; ijk < dbDict.dataObjects.length; ijk++) {
          ...
          // sync data object
          dataObjectNode.data.properties.objectNamespace = dataObject.objectNamespace;
          dataObjectNode.data.properties.objectName = dataObject.objectName;
          dataObjectNode.data.properties.keyDelimeter = dataObject.keyDelimeter;
          ...
          if (dataObject.objectName.toLowerCase() == dataObjectNode.text.toLowerCase()) {
            var keysNode = dataObjectNode.data.children[0];
            var propertiesNode = dataObjectNode.data.children[1];
            var relationshipsNode = dataObjectNode.data.children[2];
    ...
    Ext.define('dataobjectsmodel', {
      extend: 'Ext.data.Model',
      fields: []
    });

    var dataObectsStore = new Ext.data.TreeStore({
      model: 'dataobjectsmodel',
      proxy: {
        type: 'ajax',
        url: '',
        reader: { type: 'json' },
    ...
    var dataObjectsPane = new Ext.panel.Panel({
      layout: 'border',
    ...
    var showTree = function (dbObjectsTreeStore) {
      ...
      treeStore.dataUrl = 'AdapterManager/DBObjects';
      treeStore.baseParams = {
        scope: scopeName,
        app: appName,
        dbProvider: dbDict.Provider,
        dbServer: dbInfo.dbServer,
      ...
      Ext.Ajax.request({
        ...
        success: function (response, request) {
          dbTableNames = Ext.JSON.decode(response.responseText);
        },
        failure: function (f, a) {
          if (a.response)
            showDialog(500, 400, 'Error', a.response.responseText, Ext.Msg.OK, null);
      ...
      tbar: new Ext.Toolbar({
        items: [{
          ...
          handler: function () {
            ...
            if (callId == 0) {
              var dbObjectsTreeStore = dataObjectsPane.items.items[0].items.items[0];
              var node = dbObjectsTreeStore.getSelectionModel().getSelectedNode();
              var exitNode = false;
...
var addPropertyMapping = function (relationConfigPanel) {
  ...
  var dbObjectsTreeStore = dataObjectsPane.items.items[0].items.items[0];
  var gridLabel = scopeName + '.' + appName + '.' + dbObjectsTreeStore.getSelectionModel().getSelectedNode().id;
  if (dataRelationPane.items) {
    var gridPane = dataRelationPane.items.map[gridLabel];
    if (gridPane) {
      var dataStore = gridPane.store;
