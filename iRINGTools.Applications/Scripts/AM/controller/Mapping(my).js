Ext.define('AM.controller.Mapping', {
    ...
    onEditOrNewGraph: function (item, e, eOpts) {
        ...
        if (record && record.classTemplateMaps[0])
        ...
        if (record && record.classTemplateMaps[0]) {
            ...
            contextName = node.data.id.split('/')[0];//node.parentNode.parentNode.parentNode.data.text;
            endpoint = node.data.id.split('/')[1];//node.parentNode.parentNode.data.text;
        }
        else {
            contextName = node.data.id.split('/')[0];//node.parentNode.parentNode.data.text;
            endpoint = node.data.id.split('/')[1];//node.parentNode.data.text;
        }
    ...
    onDeleteGraphMap: function (item, e, eOpts) {
		...
		var application = node.data.id.split('/')[1];
        Ext.Ajax.request({
            url: 'mapping/deletegraphmap',
            method: 'POST',
            params: {
                scope: scope,//node.parentNode.parentNode.parentNode.data.text, //node.data.property.context,
                application: application,//node.parentNode.parentNode.data.text, //node.data.property.endpoint,
                baseUrl: node.data.property.baseUrl,
                mappingNode: node.id,
