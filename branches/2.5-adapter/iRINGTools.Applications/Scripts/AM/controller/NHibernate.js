...
Ext.define('AM.controller.NHibernate', {
  ...
  onSaveDataObjects: function(button, e, eOpts) {
    ...
    treeStore.on('beforeload', function (store, operation) {
      ...
      params.contextName = dirNode.data.record.ContextName;
  ...
  getDbDictionary: function(context, endpoint, baseUrl, successCallback) {
    ...
    var dbDict, dbInfo;
    Ext.Ajax.request({
      url: 'AdapterManager/DBProviders',//'NHibernate/DBDictionary',
      method: 'POST',
      timeout: 6000000,
      params: {
        contextName: context,
  ...
  showSelectTablesForm: function(nhibernatePanel) {
    ...
    */
    ...
    //itemSelecter.items.items[2].buttons = ["add","remove"];
    //itemSelecter.items.items[2].doLayout();
  ...
  showSelectKeyFieldsForm: function(nhibernatePanel) {
    ...
    var context = dirNode.data.record.ContextName;//dirNode.data.record.context;
    var endpoint = dirNode.data.record.Endpoint;//dirNode.data.record.endpoint;
    var baseUrl = dirNode.data.record.baseUrl;
  ...
  getFolderFromChildNode: function(folderNode) {
    ...
    if (folderNode.raw.properties.keyDelimiter && folderNode.raw.properties.keyDelimiter != 'null')
    folderNode.raw.properties.keyDelimiter = folderNode.raw.properties.keyDelimiter;    
    else
