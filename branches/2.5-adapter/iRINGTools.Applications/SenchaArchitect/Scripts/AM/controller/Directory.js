...
Ext.define('AM.controller.Directory', {
  ...
  newOrEditScope: function(item, e, eOpts) {
    var me = this;
    var state, context, description, wintitle;
    ...
    if(item.itemId !== 'newscope' && node.data.record !== undefined) {
    ...
    form.getForm().findField('Name').setValue(context);
  ...
  newOrEditEndpoint: function(item, e, eOpts) {
    var me = this;
    var name, description, datalayer, assembly,showconfig,endpoint,wintitle, state, path, context;
    var tree = me.getDirTree();
    var node = tree.getSelectedNode();


    if(item.itemId == 'editendpoint') {
      name = node.data.record.ContextName;
      description = node.data.record.Description;
