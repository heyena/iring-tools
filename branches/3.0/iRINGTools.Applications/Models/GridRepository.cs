...
namespace iRINGTools.Web.Models
{
    public class GridRepository : AdapterRepository, IGridRepository
    {
        ...
        public StoreViewModel GetGrid(DataObject dataObject, string start, string limit)
        {
            try
            {
                GetDataItems(dataObject, start, limit);
        ...
        private void GetDataItems(DataObject dataObject, string start, string limit)
        {
            try
            {
                string format = "json";
                DataFilter dataFilter = dataObject.dataFilter;
                //string relativeUri = "/" + app + "/" + scope + "/" + graph + "/filter?format=" + format + "&start=" + start + "&limit=" + limit;
                string relativeUri = "/getDataForDataObject?format=" + format + "&start=" + start + "&limit=" + limit;
                ...
                {
                    dataItemsJson = client.Post<DataObject, string>(relativeUri, dataObject, format, true);
                }
