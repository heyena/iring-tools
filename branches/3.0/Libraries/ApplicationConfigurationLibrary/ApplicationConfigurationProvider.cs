...
namespace org.iringtools.applicationConfig
{
    public class ApplicationConfigurationProvider : BaseProvider
    {
        ...
        public Response DeleteValueListMap(string valueListMapId)
        {
            Response response = new Response();
            response.Messages = new Messages();
            ...
            try
            {
                using (var dc = new DataContext(_connSecurityDb))
                {
                    if (string.IsNullOrEmpty(valueListMapId))
                        PrepareErrorResponse(response, "Please enter valueListMapId!");
                    else
                    {
                        NameValueList nvl = new NameValueList();
                        nvl.Add(new ListItem() { Name = "@ValueListMapid", Value = valueListMapId });

                        string output = DBManager.Instance.ExecuteScalarStoredProcedure(_connSecurityDb, "spdValueListMap", nvl); 

                        switch (output)
                        {
                            case "1":
                                PrepareSuccessResponse(response, "valuelistmapdeleted");
                                break;
                            default:
                                PrepareErrorResponse(response, output);
                                break;
        ...
        public Response UpdateValueMap(XDocument xml)
        {
            Response response = new Response();
            response.Messages = new Messages();
            ...
            try
            {
                ...
                {
                    if (valueMap == null || string.IsNullOrEmpty(valueMap.internalValue))
                        PrepareErrorResponse(response, "Please enter internal value!");
                    else
                    {
                        NameValueList nvl = new NameValueList();
                        nvl.Add(new ListItem() { Name = "@ValueListMapId", Value = valueMap.ValueListMapId });
                        nvl.Add(new ListItem() { Name = "@ValueMapId", Value = valueMap.ValueMapId });
                        nvl.Add(new ListItem() { Name = "@InternalValue", Value = valueMap.internalValue });
                        nvl.Add(new ListItem() { Name = "@Label", Value = valueMap.label });
                        nvl.Add(new ListItem() { Name = "@URI", Value = valueMap.uri });


                        string output = DBManager.Instance.ExecuteScalarStoredProcedure(_connSecurityDb, "spuValueMap", nvl);

                        switch (output)
                        {
                            case "1":
                                PrepareSuccessResponse(response, "valuemapupdated");
                                break;
                            case "0":
                                PrepareSuccessResponse(response, "duplicatevaluemap");
                                break;
