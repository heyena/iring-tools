    ...
    public class NHibernateDataLayer : IDataLayer
    {
        ...
        public IList<IDataObject> Create(string objectType, IList<string> identifiers)
        {
            try
            {
                ...
                type = Type.GetType("org.iringtools.adapter.datalayer.proj_" + _settings["Scope"] + "." + objectType + ", " + _settings["ExecutingAssemblyName"]);
               
                if (type == null) // Call from NUnit
                {
                    type = GetGlobalType("org.iringtools.adapter.datalayer.proj_12345_000.DEF.LINES");
                }
