...
public class DirectoryProvider
{
  ...
  public DirectoryProvider(Map<String, Object> settings)
  {
    ...
    path = settings.get("basePath").toString().concat("WEB-INF/data/directory.xml");
  }
...
/*	public Directory getDirectory() {
 ...
 path = settings.get("basePath").toString().concat("WEB-INF/data/directory.xml");
	...
	public Exchange postExchageDefinition(Exchange exchange,String scope, String name) {
		...
		try {
			if (IOUtils.fileExists(path)) {
				...
				for (Scope scopeDirectory : directorytList) {
					//ApplicationData appData = scopeDirectory.getApplicationData();
					
					if (scope.equalsIgnoreCase(scopeDirectory.getName()))
					{
						DataExchanges exchangeData = scopeDirectory.getDataExchanges();
						List<Commodity> commodityList = exchangeData.getCommodity();				
	...
	public void editExchageDefinition(Exchange exchange,String scope, String name, String oldConfigName) {
	...
	public Exchange gettingCommodityConfig(String comm ,String scope, String name) {
	...
	public synchronized void postDataFilterExpressions(String scope, String name,
			...
			if (IOUtils.fileExists(path)) {
				...
				for (Scope scopeDirectory : directorytList) {
					...
					if (scope.equalsIgnoreCase(scopeDirectory.getName()))
					{
						...
						//		exchange.setDataFilter(df); //exchangeList.add(exchange);
							 }
							}
	...
	public void postEditedScope(String newScope, String oldScope) {
		 ...
		 path = settings.get("basePath").toString().concat("WEB-INF/data/directory.xml");try {
			if (IOUtils.fileExists(path)) {
				Directory directory = JaxbUtils.read(Directory.class, path);
				List<Scope> directorytList = directory.getScope();
				for (Scope scopefromfile : directorytList) {
	...
	try {
			if (IOUtils.fileExists(path)) {
				...
				for (Scope scopeDirectory : directorytList) {
					...
					if (scope.equalsIgnoreCase(scopeDirectory.getName()))
					{
						ApplicationData applicationData = scopeDirectory.getApplicationData();
	...
	public org.iringtools.directory.Graph gettingGraph(String graph,String scope, String appName) {
		 path = settings.get("basePath").toString().concat("WEB-INF/data/directory.xml");
		try {
			if (IOUtils.fileExists(path)) {
				Directory directory = JaxbUtils.read(Directory.class, path);
