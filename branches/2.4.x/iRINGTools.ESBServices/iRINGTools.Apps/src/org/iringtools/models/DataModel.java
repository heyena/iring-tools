...
public class DataModel
{
  ...
  public DataModel(DataMode dataMode, String refDataServiceUri, FieldFit fieldFit, boolean isAsync, long timeout,
      long interval)
  {
  ...
  protected DataTransferIndices getDtis(String serviceUri, String manifestRelativePath, String dtiRelativePath,
      String filter, String sortBy, String sortOrder, String dataFilterRelativePath) throws DataModelException
  {
    ...
    try
    {
      ...
      if (dataFilterRelativePath != null)
      {
        ...
        try
        {
          dataFilterFile = httpClient.get(DataFilter.class, dataFilterRelativePath);
        ...
        /*
         * // merging File Filter and UI Filter DataFilterInitial dFI = new DataFilterInitial(); dataFilter =
    ...
    else
    {
      ...
      try
      {
        ...
        if (dataMode == DataMode.EXCHANGE)
        {
          ...
          //OrderExpression transferTypeOrderExpression = removeTransfertypeOrderExpression(dataFilterFile);
          // pass filter to dxiRequest to get filtered dti data.
          dxiRequest.setDataFilter(dataFilterFile);
          if (isAsync)
          {
            httpClient.setAsync(true);
            String statusUrl = httpClient.post(String.class, dtiRelativePath, dxiRequest);
            fullDtis = waitForRequestCompletion(DataTransferIndices.class, serviceUri + statusUrl);
          }
          else
          {
            fullDtis = httpClient.post(DataTransferIndices.class, dtiRelativePath, dxiRequest);
          }
          // apply filterfile on fullDtis list
          List<DataTransferIndex> tmpFullDtiList = fullDtis.getDataTransferIndexList().getItems();
  ...
  protected List<Expression> removeTransfertypeExpression(DataFilter dataFilter)
  {
    ...
    {
      if (dataFilter.getExpressions().getItems().size() == dataFilterUI.getExpressions().getItems().size())
      {
        filterFile = "no";
      }
    }
    ...
    if (dataFilter.getExpressions() != null && dataFilter.getExpressions().getItems().size() > 0)
    {
      ...
      for (; i < expressions.size(); i++)
      {
        ...
        if (expression.getPropertyName().equalsIgnoreCase("transfer type"))
        {
          ...
          String value = expressions.get(i).getValues().getItems().get(0);
          RelationalOperator relationalOper = expressions.get(i).getRelationalOperator();

          // To removing filter with type transfer type.
          if (dataFilterUI.getExpressions() != null)
          {
            List<Expression> expressionsUi = dataFilterUI.getExpressions().getItems();
            int k = 0;
            for (; k < expressionsUi.size(); k++)
            {
              if (expressionsUi.get(k).getPropertyName().equalsIgnoreCase("transfer type"))
              {
                // compare UI filter with file filter if they
                // are same remove one.
                valueUi = expressionsUi.get(k).getValues().getItems().get(0);
                if (filterFile != "no")
                {
                  if (value.equalsIgnoreCase(valueUi))
                  {
                    if (relationalOper.equals(expressionsUi.get(k).getRelationalOperator()))
                    {
                      if (count < 1)
                      {
  ...
  protected OrderExpression removeTransfertypeOrderExpression(DataFilter dataFilter)
  {
  ...
  protected List<DataTransferIndex> getTransferTypeDtis(List<DataTransferIndex> tmpFullDtiList,
      List<Expression> transferTypeExpression)
  {
    ...
    if (transferTypeExpression != null)
    {
      for (int j = 0; j < transferTypeExpression.size(); j++)
      {
        if (transferTypeExpression != null && tmpFullDtiList != null)
        {
          ...
          for (; t < tmpFullDtiList.size(); t++)
          {
            ...
            {
              if (tmpFullDtiList.get(t).getTransferType().toString().equalsIgnoreCase(value))
              {
          ...
          {
            if (tmpFullDtiList.size() != TranferNotEqualDtiList.size())
            {
              TranferNotEqualDtiList.removeAll(TranferNotEqualDtiList);
              TranferNotEqualDtiList.addAll(tmpFullDtiList);
            }
            if (transferTypeExpression.get(j).getRelationalOperator() == RelationalOperator.NOT_EQUAL_TO)
            {
              if (TranferNotEqualDtiList.get(i).getTransferType().toString().equalsIgnoreCase(value))
              {
                has_notequal = "yes";
                int removeIndex = i--;
      ...
      for (int l = 0; l < TranfertmpDtiList.size(); l++)
      {
        ...
        {
          if (TranfertmpDtiList.get(l).equals(TranferNotEqualDtiList.get(k)))
          {
      ...
      for (int l = 0; l < TranferNotEqualDtiList.size(); l++)
      {
        ...
        {
          if (TranferNotEqualDtiList.get(l).equals(TranfertmpDtiList.get(k)))
          {
      ...
      if (dataFilterUI.getExpressions() != null)
      {
        List<Expression> expressions = dataFilterUI.getExpressions().getItems();
        int i = 0;
        for (; i < expressions.size(); i++)
        {
          if (expressions.get(i).getPropertyName().equalsIgnoreCase("transfer type"))
          {
            String value = expressions.get(i).getValues().getItems().get(0);
            for (int j = 0; j < tmpFullDtiList.size(); j++)
            {
              ...
              {
                if (!tmpFullDtiList.get(j).getTransferType().toString().equalsIgnoreCase(value))
                {
      ...
      String dtiRelativePath, String serviceUri, String fullDtiKey) throws DataModelException
  {
    ...
    try
    {
      ...
      /*
       * // edited here to send the dups dti to fetch dto's List<DataTransferIndex> tmpFullDtiList = fullDtis
        ...
        else if (dataFilter.getOrderExpressions() != null && dataFilter.getOrderExpressions().getItems().size() > 0)
        {
          ...
          Comparator<DataTransferIndex> comparator = new Comparator<DataTransferIndex>()
          {
            public int compare(DataTransferIndex dti1, DataTransferIndex dti2)
            {
              ...
              /*
               * } else // sort type is numeric { if (dti1SortIndex == null) { dti1SortIndex = String
               * .valueOf(Double.MAX_VALUE); }
               * 
               * if (dti2SortIndex == null) { dti2SortIndex = String .valueOf(Double.MAX_VALUE); }
               * 
               * if (sortDir.equals("asc")) { compareValue = (int) (Double .parseDouble(dti1SortIndex) - Double
               * .parseDouble(dti2SortIndex)); } else { compareValue = (int) (Double .parseDouble(dti2SortIndex) -
               * Double .parseDouble(dti1SortIndex)); } }
               */
  ...
  protected Grid getDtoGrid(String fieldsContext, Manifest manifest, Graph graph, DataTransferObjects dtos)
      throws DataModelException
  {
    ...
    if (graph != null)
    {
      ...
      for (int dtoIndex = 0; dtoIndex < dtoList.size(); dtoIndex++)
      {
         ...
         * 
         * String dups; if (dto.getDuplicateCount() == null) { dups = "1"; rowData.add(dups); } else
  ...
  private List<Field> createFields(Graph graph, String startClassId)
  {
    ...
    /*
     * // Dups count field Field dupField = new Field(); dupField.setName("Dup's"); dupField.setDataIndex("Dups Count");
     * dupField.setType("string"); dupField.setWidth(MIN_FIELD_WIDTH); dupField.setFixed(true);
     * dupField.setFilterable(false); fields.add(0, dupField);
     */
