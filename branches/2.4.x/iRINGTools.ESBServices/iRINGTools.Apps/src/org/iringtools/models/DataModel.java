...
public class DataModel {
	...
	protected DataTransferIndices getDtis(String serviceUri,
		...
		try {
			...
			if (dataFilter != null
				...
				// filtered data
				{  
					/*// checking whether the applied UI filter is eligible or not		
	...
	protected Grid getDtoGrid(String fieldsContext, Manifest manifest,
		...
		if (graph != null) {
			...
			for (int dtoIndex = 0; dtoIndex < dtoList.size(); dtoIndex++) {
				 ...
				 * 
				 * String dups; if (dto.getDuplicateCount() == null) { dups =
				 * "1"; rowData.add(dups); } else
		...
		for (DataTransferIndex dti : dtiList) {
			if (dti.getIdentifier().equals(dtoIdentifier)) {
				...
				try {
					...
					relatedDtos = httpClient.post(DataTransferObjects.class,
							dtoRelativePath, dxoRequest);
	...
	private List<Field> createFields(Graph graph, String startClassId) {
		...
		/*
		 * // Dups count field Field dupField = new Field();
		 * dupField.setName("Dup's"); dupField.setDataIndex("Dups Count");
		 * dupField.setType("string"); dupField.setWidth(MIN_FIELD_WIDTH);
		 * dupField.setFixed(true); dupField.setFilterable(false); fields.add(0,
		 * dupField);
		 */
	...
	protected <T> T waitForRequestCompletion(Class<T> clazz, String url) {
		...
		try {
			...
			obj = (T) JaxbUtils
					.toObject(clazz, requestStatus.getResponseText());
		} catch (Exception ex) {
