...
AdapterManager.DirectoryPanel = Ext.extend(Ext.Panel, {
    ...
    */
    ...
    onRefreshCache: function (btn, ev) {
        var me = this;
        showDialog(250, 40, 'Please Confirm..', 'Do you want to Refresh Cache?', Ext.Msg.YESNO, function (option) {
            if (option === 'no')
                return;

            var node = me.directoryPanel.getSelectionModel().getSelectedNode();
            Ext.getCmp('content-panel').getEl().mask('Processing...', 'x-mask-loading');
    ...
    onImportCache: function (btn, ev) {
        var me = this;
        showDialog(250, 40, 'Please Confirm..', 'Do you want to Import Cache?', Ext.Msg.YESNO, function (option) {
            if (option === 'no')
                return;
            var node = me.directoryPanel.getSelectionModel().getSelectedNode();
            var importCacheURI = node.attributes.property['Cache URI'];
            var cacheTimeout = node.attributes.property['Cache Timeout'];
