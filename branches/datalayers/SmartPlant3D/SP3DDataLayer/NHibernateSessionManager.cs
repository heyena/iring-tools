...
namespace iringtools.sdk.sp3ddatalayer
{
  public sealed class NHibernateSessionManager
  {
    ...
    public ISession GetSessionSP3D(string path, string context, string commodityName)
    {
      try
      {
        lock (_lockObj)
        {
          string factoryKey = (context + '.' + commodityName).ToLower();
    ...
    public ISession CreateCachingTables(string path, string context, string businessCommodityName)
    {
      try
      {
        lock (_lockObj)
    ...
    private void InitSessionFactorySP3D(string path, string context, string commodityName)
    {
      try
      {
        lock (_lockObj)
        {
          string cfgPath = string.Format("{0}nh-configuration.{1}.{2}.xml", path, context, commodityName);
          string mappingPath = string.Format("{0}nh-mapping.{1}.{2}.xml", path, context, commodityName);
          string connStr = "";
