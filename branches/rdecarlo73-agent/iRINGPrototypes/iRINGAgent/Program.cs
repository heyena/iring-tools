...
namespace org.iringtools.agent
{
    class Program
    {
        ...
        static void GenerateSampleSequence()
        {
            AgentSequence sequence = new AgentSequence
            {
                Name = "Test",
                Tasks = new List<Task>
                {
            ...
            Stream stream = Utility.ReadStream("SampleSequence.dat");

            sequence = Utility.DeserializeBinaryStream<AgentSequence>(stream);

            Utility.Write<AgentSequence>(sequence, "SampleSequence.xml", true);
        }
