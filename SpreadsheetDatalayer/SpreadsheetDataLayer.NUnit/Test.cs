...
namespace iRINGTools.SDK.SpreadsheetDataLayer.NUnit
{
  ...
  public class SpreadsheetDataLayerTest
  {
    ...
    private org.iringtools.adapter.datalayer.SpreadsheetDataLayer _dataLayer;
    ...
    public SpreadsheetDataLayerTest()
    {
      ...
      if (_settings["TestMode"].ToLower() != "usefiles")
      {
        ...
        Utility.Write<SpreadsheetConfiguration>(configuration, @"App_Data\spreadsheet-configuration.12345_000.SPDS.xml");
      }      

      _dataLayer = new org.iringtools.adapter.datalayer.SpreadsheetDataLayer(_adapterSettings);
    }

    [Test]
    public void GetAll()
    {
        var ids = _dataLayer.GetIdentifiers("Lines", null);
        var recs = _dataLayer.Get("Lines", ids);
        if (recs.Count == 0)
        { 
        }
    }
