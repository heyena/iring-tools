...
rem Create iring database and user
"%mySQLCmd%" --user=root --password=%dbPassword% <iring.sql 2>temp.txt
...
rem Create iRING Interface Service Root environment variable
setx IRING_INTERFACE_ROOT %~dp0% >nul 2>nul
if %errorlevel% neq 0 (
  rem Require logout to take in effect
  reg add HKCU\Environment /v IRING_INTERFACE_ROOT /d %~dp0% /f >nul
)

rem Create iRING Interface Service
@echo Creating iRING Interface Service ...
sc query interfacesvc | findstr /i "service_name" >nul
if %errorlevel% equ 0 (
  rem Delete old service
  sc stop interfacesvc >nul
  :wait
  ping -n 1 127.0.0.1 >nul
  tasklist | find "iifwrapper.exe" >nul
  if not %errorlevel% equ 1 goto :wait
  sc delete interfacesvc >nul
)

iifwrapper.exe -i joseki-service.conf
sc start interfacesvc >nul
@echo iRING Interface Service started.
