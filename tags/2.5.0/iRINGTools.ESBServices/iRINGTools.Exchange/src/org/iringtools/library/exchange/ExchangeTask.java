      ...
      else
      {
        ...
        else
        {
          ...
          for (DataTransferIndex dxi : dtis.getDataTransferIndexList().getItems())
          {
            ...
            {
              String message = "Excluding DTO [" + dxi.getIdentifier() + "] due to [" + dxi.getDuplicateCount()
                  + "] duplicates. ";
              logger.warn(message);
          ...
          //
          String targetAppUrl = exchange.getTargetUri() + "/" + exchange.getTargetScope() + "/"
              + exchange.getTargetApp();
          String targetGraphUrl = targetAppUrl + "/" + exchange.getTargetGraph();
          String sourceGraphUrl = exchange.getSourceUri() + "/" + exchange.getSourceScope() + "/"
              + exchange.getSourceApp() + "/" + exchange.getSourceGraph();
              ...
              if (sourceDtos != null && sourceDtos.getDataTransferObjectList() != null)
              {
                for (DataTransferIndex dti : sourceDtiItems)
                {
                  for (DataTransferObject dto : sourceDtos.getDataTransferObjectList().getItems())
                  {
                    ...
                    {
                      dto.setTransferType(org.iringtools.dxfr.dto.TransferType
                          .valueOf(dti.getTransferType().toString()));
              ...
              if (poolDtoListItems.size() > 0)
              {
                ...
                if (poolResponse != null)
                {
                  try
                  {
                    ...
                    String poolResponseFile = exchangeFile + Constants.XITEM_PREFIX + (i + 1) + "-"
                        + (i + actualPoolSize) + ".xml";
