...
ExchangeManager.DirectoryPanel = Ext.extend(Ext.Panel, {
  ...
  initComponent : function() {
    ...
    this.items = [ this.directoryPanel, this.propertyPanel ];
  ...
  openTab : function(node, isReloadable) {
    ...
    /*
     * loadAppData(dataTypeNode.parentNode.attributes['text'],
     * graphNode.attributes['text'], node.attributes['text']); function
    ...
    if (node != null) {
      ...
      if ((nodeType == 'exchange' && uid != '')) {
        ...
        // requestURL = 'dataObjects/getDataObjects/' + nodeType + '/' + scopeId
        // + '/' + uid;
        ...
        // requestURL = 'dataObjects/getGraphObjects/' + nodeType + '/' +
        // scopeId + '/' + node.parentNode.text+'/' + nodeText;
    ...
    if (node != null) {
      ...
      if (nodeType == 'exchange') {
        ...
        if (contentPanel.get(tabid) == undefined) {
          ...
          /*
           * var
           * exchangeHistoryURI='exchangeHistory.json?scopeName='+scopeId+'&idName='+uid;
           * alert(exchangeHistoryURI) directoryPan.fireEvent('history', this,
           * node,exchangeHistoryURI,scopeId,uid);
           */
    ...
    if (node != null) {
      ...
      if (nodeType == 'exchange') {
        ...
        if (contentPanel.get(tabid) == undefined) {
          ...
          Ext.Msg.show( {
            ...
            fn : function(action) {
              if (action == 'yes') {
                ...
                hasreviewed = false;
                contentPanel.setActiveTab(tabid);
                var exchangeURI = 'exchangeResponse?scopeName=' + scopeId + '&idName=' + uid + '&hasReviewed='
                    + hasreviewed;
          ...
          // condition when the tabPanel is open and user clicks the exchange
          // button
          hasreviewed = true;
          contentPanel.setActiveTab(tabid);
