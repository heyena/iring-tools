...
namespace org.iringtools.services
{
    public class CommonDataService
    {
        ...
        private ApplicationConfigurationProvider appConfigProvider = null;

        public CommonDataService()
        {
            _adapterProvider = new AdapterProvider(ConfigurationManager.AppSettings);
            appConfigProvider = new ApplicationConfigurationProvider(ConfigurationManager.AppSettings);
        }
        ...
        public void GetContexts(string app, string format)
        {
            ...
            org.iringtools.library.Contexts contexts = _adapterProvider.GetContexts(app);

            _adapterProvider.FormatOutgoingMessage<org.iringtools.library.Contexts>(contexts, format, true);
        }
        ...
        public void GetDataForDataObject(string format, int start, int limit, string indexStyle, Stream stream)
        {
            try
            {
                ...
                if (IsAsync())
                {
                    //string statusUrl = _adapterProvider.AsyncGetWithFilter(dataObject.dataObjectId, format, start, limit, fullIndex, filter);
                    WebOperationContext.Current.OutgoingResponse.StatusCode = HttpStatusCode.Accepted;
                    WebOperationContext.Current.OutgoingResponse.Headers["location"] = "statusUrl";
                }
                else
                {
                    DataObject dataObject = _adapterProvider.FormatIncomingMessage<DataObject>(stream, format, true);
                    XDocument xDocument = null;

                    xDocument = appConfigProvider.GetDataForDataObject(dataObject, ref format, start, limit, fullIndex);
