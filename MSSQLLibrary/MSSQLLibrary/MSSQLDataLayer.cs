  ...
  public class MSSQLDataLayer : BaseDataLayer
  {
   ...
   // private AdapterSettings _settings = null;
    ...
    public MSSQLDataLayer(AdapterSettings settings):base(settings)
    {
      ...
      _msSQLconfiguration = GetConfig(projectName, applicationName);
    }
    ...
    public override Response Delete(string objectType, IList<string> identifiers)
    {
      ...
      try
      {
        ...
        foreach (string identifier in identifiers)
        {
          ...
          {
              sql.AppendLine(string.Format(" FROM {0}", objectSql.ObjectName));
    ...
    public override Response Post(IList<IDataObject> dataObjects)
    {
      ...
      try
      {
        foreach (IDataObject dataObject in dataObjects)
        {
          ...
          if (objectSql == null)
          {
            objectSql = _msSQLconfiguration.FirstOrDefault(c => c.ObjectTypeName == primaryObject);
    ...
    {
      return _msSQLconfiguration.FirstOrDefault(c => c.ObjectTypeName == objectTypeName);
    }
    ...
    public Response SaveConfiguration(XElement configuration)
    {
      ...
      try
      {
        _msSQLconfiguration = Utility.DeserializeFromXElement<MSSQLConfiguration>(configuration);
        Utility.Write<MSSQLConfiguration>(_msSQLconfiguration, _xmlPath, true);
