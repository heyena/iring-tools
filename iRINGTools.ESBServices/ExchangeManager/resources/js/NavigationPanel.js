...
fields: ['name','reference','identifier']
});
...
var listView = new Ext.list.ListView({
	...
	columns: [{header: 'Class Name',dataIndex:'name'},{header: 'reference',hidden: true},{header: 'identifier',hidden:true}]
});

listView.on('click', function( dataView, index, node, e ) {
	//**** alert(this.configData.relatedClasses[rowIndex].identifier)
	alert(dataView.store.data.items[index].data.identifier)
	alert(dataView.store.data.items[index].data.reference)
	alert(dataView.store.data.items[index].data.name)
	...
	*/
	...
	/*var newTab = {
			title: dataView.store.data.items[index].data.identifier,
			id:this.title+'_'+dataView.store.data.items[index].data.identifier,
			//items:[classPanel],
			html:'aaaaaaaa',
			closable : true
			};
...
layoutConfig: {animate: true,fill : false},
					...
					// html: this.configData.relatedClasses[rowIndex].text
					//html: this.configData.relatedClasses[rowIndex].label
items:listView
	   }
	   ]
			   });
