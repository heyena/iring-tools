...
public class ExchangeDataModel extends DataModel {
	private static final Logger logger = Logger
			.getLogger(ExchangeDataModel.class);
	private ExchangeProvider provider;
	private DirectoryProvider dprovider;

	public ExchangeDataModel(Map<String, Object> settings,
	...
	public Grid getDtoGrid(String serviceUri, String scope, String xId,
		...
		//
		// store the last filter being applied DTIs so exchange submission will
		// not need to provide
		//
		...
		DataTransferIndices dtis = getDtis(exchange, manifest, filter,
				sortOrder, sortBy);
		...
		if (dtis != null && dtis.getDataTransferIndexList() != null) {
			...
			List<DataTransferIndex> pageDtis = dtiList.subList(start,
					actualLimit);

			//
			// send a page of DTIs to get DTOs
			//
			DataTransferObjects dtos = provider.getDataTransferObjects(
					exchange, manifest, pageDtis);

			//
			// remove duplicate DTOs if any to show only one representative DTO
			//
			if (dtos != null && dtos.getDataTransferObjectList() != null) {
				if (dtiList.size() < dtos.getDataTransferObjectList()
						.getItems().size()) {
					List<DataTransferObject> dtoList = dtos
				...
				dtoGrid = createDtoGrid(serviceUri, dtiRelativePath, manifest,
						graph, dtos);
	...
	public Map<String, String> getPreSummaryGrid(String serviceUri,
			String scope, String xId) throws Exception {
		this.scope = scope;
		this.xId = xId;
	...
	public ExchangeResponse submitExchange(String serviceUri, String scope,
			String xId, boolean reviewed) throws Exception {
		this.scope = scope;
		this.xId = xId;
		...
		try {
			...
			String noDocTypeError = noHtmlError.replaceAll(
					"(<!DOCTYPE)[^&]*(>)", "");
	...
	public RequestStatus getExchangeRequestStatus(String scope, String xId)
			throws Exception {
		// TODO get exchange status from provider and return;
		RequestStatus requestStatus = provider.getExchangeRequestStatus(scope,
				xId);
		return requestStatus;
	}

	public Grid getXHistoryGrid(String scope, String xId, String xlabel)
			throws Exception {
		...
		if (xHistory != null && xHistory.getExchangeResponses().size() > 0) {
			...
			for (ExchangeResponse xr : xrs) {
				...
				row.add("<input type=\"image\" src=\"resources/images/info-small.png\" "
						...
						+ xTime
						+ "\","
						+ xr.getPoolSize() + "," + xr.getItemCount() + ")'>");

				row.add(formattedStartTime);
				row.add(format(endTime));
				row.add(xr.getSenderScope() + "." + xr.getSenderApp() + "."
						+ xr.getSenderGraph());
				row.add(xr.getReceiverScope() + "." + xr.getReceiverApp() + "."
						+ xr.getReceiverGraph());
				String summary = xr.getSummary();
				// String resultSummary = summary.replaceAll("\\<.*?\\>", "");
				String resultSummary = summary.replaceAll(
						"(<html)[^&]*(</html>)", "");
				String romoveDoc = resultSummary.replaceAll(
						"(<!DOCTYPE)[^&]*(>)", "");
	...
	public Grid getPageXHistoryGrid(String scope, String xId, String xtime,
			int start, int limit, int itemCount) throws Exception {
		Grid pageXHistoryGrid = new Grid();

		int actualLimit = (start + limit > itemCount) ? (itemCount - start)
				: limit;
		Response response = provider.getExchangeResponse(scope, xId, xtime,
				start, actualLimit);
	...
	// merge data filter expressions, remove duplicate expressions and exclude
	// transfer type expressions
	protected DataFilter mergeDataFilters(DataFilter filter1,
			DataFilter filter2, List<Expression> transferTypeExpressions) {
		...
		if (filter1 != null) {
			if (filter1.getExpressions() != null) {
				for (Expression expression : filter1.getExpressions()
						.getItems()) {
					if (expression.getPropertyName().equalsIgnoreCase(
							"transfer type")) {
						transferTypeExpressions.add(expression);
					} else {
						if (expressions.getItems().size() > 0
								&& expression.getLogicalOperator() == null)
			...
			if (filter1.getOrderExpressions() != null) {
				for (OrderExpression orderExpression : filter1
		...
		if (filter2 != null) {
			if (filter2.getExpressions() != null) {
				for (Expression expression : filter2.getExpressions()
						.getItems()) {
					if (expression.getPropertyName().equalsIgnoreCase(
							"transfer type")) {
			...
			if (filter2.getOrderExpressions() != null) {
				for (OrderExpression orderExpression : filter2
						.getOrderExpressions().getItems()) {
	...
	protected void applyTransferTypeFilterToDtis(DataTransferIndices dtis,
			List<Expression> transferTypeExpressions) {
		if (transferTypeExpressions != null
				&& transferTypeExpressions.size() > 0 && dtis != null
				&& dtis.getDataTransferIndexList() != null
				&& dtis.getDataTransferIndexList().getItems().size() > 0) {
			List<DataTransferIndex> indices = dtis.getDataTransferIndexList()
					.getItems();
	...
	protected DataFilter separateTransferTypeExpressions(DataFilter inFilter,
			List<Expression> transferTypeExpressions) {
		...
		if (inFilter != null) {
			...
			if (inFilter.getExpressions() != null) {
				for (Expression expression : inFilter.getExpressions()
						.getItems()) {
					if (expression.getPropertyName().equalsIgnoreCase(
							"transfer type")) {
						transferTypeExpressions.add(expression);
	...
	protected DataTransferIndices getFullDtis(Exchange exchange,
		...
		try {
			...
			DataFilter filter = separateTransferTypeExpressions(preFilter,
					transferTypeExpressions);
			...
			logger.error("Error getting data transfer indices: "
					+ ex.toString());
	...
	protected DataTransferIndices getFilteredDtis(Exchange exchange,
			DataTransferIndices fullDtis, Manifest manifest, DataFilter uiFilter) {
		...
		try {
			...
			DataFilter preFilter = exchange.getDataFilter();
			DataFilter filter = mergeDataFilters(preFilter, uiFilter,
					transferTypeExpressions);
			...
			//
			if (filter == null
					|| (filter.getExpressions().getItems().size() == 0 && filter
							.getOrderExpressions().getItems().size() == 0)) {
				...
				List<String> ids = provider
						.getIdentifiers(exchange, dxiRequest);
				...
				if (ids != null) {
					...
					for (String id : ids) {
						for (DataTransferIndex dti : fullDtis
								.getDataTransferIndexList().getItems()) {
							if (dti.getInternalIdentifier()
									.equalsIgnoreCase(id)) // handle add,
															// delete, sync
							{
								//
								...
								// boolean found = false;
								//
								// for (DataTransferIndex existingDti :
								// dtiList.getItems())
								// {
								// if
								// (existingDti.getHashValue().equals(dti.getHashValue()))
								// {
								// found = true;
								// break;
								// }
								...
								dtiList.getItems().add(dti);
								break;
							} else if (dti.getInternalIdentifier().contains(
									Constants.CHANGE_TOKEN)) // handle change
							{
								String[] internalIds = dti
										.getInternalIdentifier().split(
												Constants.CHANGE_TOKEN);

								if (id.equals(internalIds[0])
										|| id.equals(internalIds[1])) {
									//
									// make sure no duplicates in dtiList
									//
									// boolean found = false;
			...
			if (uiFilter != null) {
				if (uiFilter.getExpressions() != null) {
					for (int i = 0; i < indices.size(); i++) {
						String transferType = indices.get(i).getTransferType()
								.toString();

						for (Expression expression : uiFilter.getExpressions()
								.getItems()) {
							if (expression.getPropertyName().equalsIgnoreCase(
									"Transfer Type")) {
								String value = expression.getValues()
										.getItems().get(0);
	...
	protected DataTransferIndices getDtis(Exchange exchange, Manifest manifest,
			String filter, String sortOrder, String sortBy) throws Exception {
		DataTransferIndices dtis = null;
		...
		if (manifest != null && manifest.getGraphs() != null) {
			...
			if (graph != null) {
				...
				if (!session.containsKey(fullDtiKey)) {
					String preSummaryKey = PRE_SUMMARY_PREFIX + "." + scope
							+ "." + xId + "." + filter;
					if (session.containsKey(preSummaryKey))
				...
				if (filter == null && sortOrder == null && sortBy == null) {
					...
					String dtiKey = DTI_PREFIX + "." + scope + "." + xId + "."
					...
					if (session.containsKey(dtiKey)) {
						...
						DataFilter uiFilter = createDataFilter(filter, sortBy,
								sortOrder);
						DataTransferIndices fullDtis = (DataTransferIndices) session
								.get(fullDtiKey);
						dtis = getFilteredDtis(exchange, fullDtis, manifest,
								uiFilter);
	...
	protected Manifest getLatestCrossedManifest(Exchange exchange)
			throws Exception {
		return provider.getCrossedManifest(exchange);
	}

	protected Manifest getCrossedManifest(Exchange exchange, String scope,
			String xId) throws Exception {
		...
		ClassTemplatesList deletedTemplateList = getExcludedTemplateList(scope,
				xId);
		filterCrossedManifest(manifest, deletedTemplateList);
		return manifest;
	}

	public void saveExcludedTemplateList(String scope, String xId)
			throws Exception {
		Exchange exchange = getExchange(scope, xId);
		ClassTemplatesList deletedTemplateList = getExcludedTemplateList(scope,
				xId);
	...
	public void reloadCrossedManifest(String scope, String xId)
			throws Exception {
		String filterKey = MANIFEST_FILTER_PREFIX + "." + scope + "." + xId;
		String manifestKey = MANIFEST_PREFIX + "." + scope + "." + xId;
		session.remove(filterKey);
	...
	public void addTemplateInDeletedList(String scope, String xId,
			String parentClassId, String parentClassIndex,
			String parentClassPath, String templateId, String templateIndex)
			throws Exception {
		List<ClassTemplates> removedTemplateList = getExcludedTemplateList(
				scope, xId).getItems();
		...
		Graph graph = manifest.getGraphs().getItems().get(0);
		List<ClassTemplates> classTemplateList = graph.getClassTemplatesList()
				.getItems();

		for (ClassTemplates classTemplate : classTemplateList) {
			if (classTemplate.getClazz() != null) {
				// if
				// ((classTemplate.getClazz().getPath().equals(parentClassPath)))
				if (classTemplate.getClazz().getId().equals(parentClassId)
						&& (classTemplate.getClazz().getPath() == null ? parentClassPath == null
								: classTemplate.getClazz().getPath()
										.equals(parentClassPath)))

				{
					List<Template> templates = classTemplate.getTemplates()
							.getItems();

					for (int j = 0; j < templates.size(); j++) {
						Template template = templates.get(j);
						if (template.getId().equals(templateId)
								&& String.valueOf(template.getIndex()).equals(
										templateIndex)) {
							boolean isClassFound = false;
							// templates.remove(j--);
							for (int k = 0; k < removedTemplateList.size(); k++) {
								Class cls = removedTemplateList.get(k)
										.getClazz();
								if (cls != null
										&& cls.getId().equals(parentClassId)
										&& (cls.getPath() == null ? parentClassPath == null
												: cls.getPath().equals(
														parentClassPath))) {
									removedTemplateList.get(k).getTemplates()
											.getItems().add(templates.get(j));
									isClassFound = true;
								}
							}
	...
	public void removeTemplateFromDeletedList(String scope, String xId,
			String parentClassId, String parentClassIndex,
			String parentClassPath, String templateId, String templateIndex)
			throws Exception {
		List<ClassTemplates> deletedTemplateList = getExcludedTemplateList(
				scope, xId).getItems();
		...
		for (int i = 0; i < deletedTemplateList.size(); i++) {
			...
			if (cls != null
					&& cls.getId().equals(parentClassId)
					&& (cls.getPath() == null ? parentClassPath == null : cls
				...
				if (templates != null) {
					for (int j = 0; j < templates.getItems().size(); j++) {
						...
						if (tmp.getId().equals(templateId)
								&& templateIndex.endsWith(String.valueOf(tmp
										.getIndex())))
	...
	public Manifest filterCrossedManifest(Manifest manifest,
			ClassTemplatesList deletedTemplateList) throws Exception {
		if (deletedTemplateList == null
				|| deletedTemplateList.getItems().isEmpty())
			return manifest;

		for (int i = 0; i < deletedTemplateList.getItems().size(); i++) {
			Class cls = deletedTemplateList.getItems().get(i).getClazz();
			Templates templatesList = deletedTemplateList.getItems().get(i)
					.getTemplates();
			...
			for (Template template : templatesList.getItems()) {
				...
				Graph graph = manifest.getGraphs().getItems().get(0);
				List<ClassTemplates> classTemplateList = graph
				...
				for (ClassTemplates classTemplate : classTemplateList) {
					if (classTemplate.getClazz() != null) {
						// if
						// ((classTemplate.getClazz().getPath().equals(parentClassPath)))
						if (classTemplate.getClazz().getId()
								.equals(parentClassId)
								&& (classTemplate.getClazz().getPath() == null ? parentClassPath == null
										: classTemplate.getClazz().getPath()
												.equals(parentClassPath))) {
							List<Template> templates = classTemplate
									.getTemplates().getItems();
							for (int j = 0; j < templates.size(); j++) {
								Template temp = templates.get(j);
								if (temp.getId().equals(templateId)
										&& String.valueOf(temp.getIndex())
												.equals(templateIndex)) {
									templates.remove(j--);
								}
							}
						}
	...
	private ClassTemplatesList getExcludedTemplateList(String scope, String xId)
			throws Exception {
	...
	public Tree getCrossedManifestTree(String scope, String xId)
			throws Exception {
		Exchange exchange = getExchange(scope, xId);
		// Manifest manifest = getCrossedManifest(exchange, scope, xId);
	...
	protected Tree manifestToTree(Manifest manifest,
			ClassTemplatesList deletedTemplateList) throws Exception {
		Tree tree = new Tree();

		try {
			Graph graph = manifest.getGraphs().getItems().get(0);
			Class rootClass = graph.getClassTemplatesList().getItems().get(0)
					.getClazz();
			...
			HashMap<String, String> graphProperties = rootClassNode
					.getProperties();
	...
	private void TemplateToTreeNode(Class parentClass,
			TreeNode parentClassNode, Graph graph,
			ClassTemplatesList deletedclsTemplateList, boolean isDeleted)
			throws Exception {
		try {
			List<ClassTemplates> classTemplateList = graph
					.getClassTemplatesList().getItems();
			for (ClassTemplates classTemplate : classTemplateList) {
				if (classTemplate.getClazz() != null && parentClass != null) {
					// if
					// (parentClass.getPath().equals(classTemplate.getClazz().getPath()))
					if (parentClass.getId().equals(
							classTemplate.getClazz().getId())
							&& (parentClass.getPath() == null ? classTemplate
									.getClazz().getPath() == null : parentClass
									.getPath().equals(
											classTemplate.getClazz().getPath()))) {
						List<Node> templateNodes = parentClassNode
								.getChildren();

						for (Template template : classTemplate.getTemplates()
								.getItems()) {
							...
							if (isDeleted) {
								...
								for (ClassTemplates deletedClassTemplates : deletedclsTemplateList
										.getItems()) {
									Class cls = deletedClassTemplates
											.getClazz();
									if (cls != null
											&& deletedClassTemplates
													.getTemplates() != null) {
										if (cls.getId().equals(
												classTemplate.getClazz()
														.getId())
												&& (cls.getPath() == null ? classTemplate
														.getClazz().getPath() == null
														: cls.getPath()
																.equals(classTemplate
																		.getClazz()
																		.getPath()))) {
											List<Template> deletedTmpList = deletedClassTemplates
													.getTemplates().getItems();
											for (Template tmp : deletedTmpList) {
												if (tmp.getId().equals(
														template.getId())
														&& tmp.getIndex() == template
																.getIndex()) {
													isTemplateDeleted = true;
													break;
												}
							...
							HashMap<String, String> templateProperties = templateNode
									.getProperties();
							templateProperties.put("Id", template.getId());
							templateProperties.put("Name", template.getName());
							templateProperties.put("TemplateIndex",
									String.valueOf(template.getIndex()));
							templateProperties.put("IsDeleted",
									String.valueOf(isTemplateDeleted));
							...
							for (Role role : template.getRoles().getItems()) {
								if (role.getClazz() != null) {
									...
									HashMap<String, String> roleProperties = roleNode
											.getProperties();
									roleProperties.put("Id", role.getId());
									roleProperties.put("IsDeleted",
											String.valueOf(isTemplateDeleted));

									// add class node
									List<Node> classNodes = roleNode
											.getChildren();
									TreeNode classNode = new TreeNode();
									classNode
											.setText(role.getClazz().getName());
									classNode.setIconCls("class");
									classNode.setType("ClassNode");
									classNodes.add(classNode);

									HashMap<String, String> classProperties = classNode
											.getProperties();
									classProperties.put("Id", role.getClazz()
											.getId());
									classProperties.put("Name", role.getClazz()
											.getName());
									classProperties.put("Path", role.getClazz()
											.getPath());
									classProperties.put("ClassIndex",
											String.valueOf(role.getClazz()
													.getIndex()));
									classProperties.put("IsDeleted",
											String.valueOf(isTemplateDeleted));

									TemplateToTreeNode(role.getClazz(),
											classNode, graph,
											deletedclsTemplateList,
											isTemplateDeleted);
								} else {
									// add leafnode
	...
	// public Grid getRelatedDtoGrid(String serviceUri, String scope, String
	// xId, String dtoIdentifier, String classId,
	// String classIdentifier, String filter, String sortBy, String sortOrder,
	// int start, int limit)
	...
	// if (graph != null)
	// {
	// DataTransferObjects dtos = getRelatedItems(serviceUri,
	// manifestRelativePath, dtiRelativePath, dtoRelativePath,
	// dtoIdentifier, filter, sortBy, sortOrder, start, limit);
	//
	// pageDtoGrid = getRelatedItemGrid(dtiRelativePath, manifest, graph, dtos,
	...
	// protected DataTransferObjects getRelatedItems(String serviceUri, String
	// manifestRelativePath, String
	// dtiRelativePath,
	// String dtoRelativePath, String dtoIdentifier, String filter, String
	// sortBy, String sortOrder, int start, int limit)
	// throws DataModelException
	// {
	...
	// dtis.getDataTransferIndexList().getItems();
	//
	// for (DataTransferIndex dti : dtiList)
	// {
	...
	// String statusUrl = httpClient.post(String.class, dtoRelativePath,
	// dxoRequest);
	//
	// try
	// {
	// relatedDtos = waitForRequestCompletion(DataTransferObjects.class,
	...
	// else
	// {
	// relatedDtos = httpClient.post(DataTransferObjects.class, dtoRelativePath,
	// dxoRequest);
	// }
	//
	// // apply filter
	...
	// relatedDtos.getDataTransferObjectList().getItems();
	//
	...
	// dto.getClassObjects().setItems(getFilteredClasses(expression,
	// classObjects));
	// }
	// }
	// }
	...
	//
	// //TODO: complete code
	// protected Grid getRelatedItemGrid(String fieldsContext, Manifest
	// manifest, Graph graph, DataTransferObjects dtos,
	...
	//
	// List<DataTransferObject> dtoList =
	// dtos.getDataTransferObjectList().getItems();
	//
	...
	// }
	//
	// processClassObject(manifest, graph, dto, dtoIndex, fields, classObject,
	...
	public void newExchangeConfig(Exchange form, String scope,
			String commodity, String serviceUri) {
		dprovider.postExchangeDefinition(form, scope, commodity);
	}

	public void editExchangeConfig(Exchange form, String scope,
			String commodity, String oldConfName, String serviceUri) {

		dprovider.editExchangeDefinition(form, scope, commodity, oldConfName);
	}

	public Exchange getCommodityConfigInfo(String com, String scope,
			String name, String serviceUri) {

		return dprovider.getCommodityConfigInfo(com, scope, name);
	}

	public void deleteExchangeConfig(String com, String scope, String name,
			String serviceUri) {
	...
	public ExchangeResponse editScope(String scope, String oldScope,
			String serviceUri) {
	...
	public ExchangeResponse newApplication(Application app, String scope,
			String serviceUri) {
	...
	public ExchangeResponse editApplication(Application app, String oldAppName,
			String scope) {
		ExchangeResponse exres = dprovider.editApplication(app, oldAppName,
				scope);
		return exres;
	}
	...
	public ExchangeResponse newGraph(org.iringtools.directory.Graph graph,
			String scope, String appname, String serviceUri) {
		ExchangeResponse exres = dprovider.postGraph(graph, scope, appname);
		return exres;
	}

	public ExchangeResponse editGraph(org.iringtools.directory.Graph graph,
			String scope, String appname, String oldGraphName, String serviceUri) {
		ExchangeResponse exres = dprovider.editGraph(graph, scope, appname,
				oldGraphName);
		return exres;
	}
	...
	public ExchangeResponse newCommodity(Commodity com, String scope,
			String serviceUri) {
		ExchangeResponse exRes = dprovider.postCommodity(com, scope);
		return exRes;
	}

	public ExchangeResponse editCommodity(Commodity com, String scope,
			String oldCommName, String serviceUri) {
		ExchangeResponse exRes = dprovider.editCommodity(com, scope,
				oldCommName);
		return exRes;
	}
	...
	public Commodity getCommodityInfo(String com, String scope,
			String serviceUri) {
	...
	public List<List<String>> getColumns(String exchangeServiceUri,
			String scope, String xid, String sort) throws Exception {
		this.scope = scope;
		this.xId = xid;
		...
		for (Field field : fields) {
			...
			if (sort.equalsIgnoreCase("true")) {
				if (field.getName() != "Status" && field.getName() != "Content"
						&& field.getName() != "Info"
	...
	public List<List<String>> getInternalScopeNameFromAM(String Uri)
			throws IOException {
		...
		try {
			...
			List<org.iringtools.library.Scope> scopes = adapterMangerScopes
					.getItems();
		...
		try {
			...
			for (org.iringtools.library.Scope s : scopes) {
				if (s.getName().equalsIgnoreCase(appScope)) {
					...
					List<org.iringtools.library.Application> appNamelist = apps
							.getItems();
						...
						if (app.getName().equalsIgnoreCase(name)) {
							if (app.getDataMode() != null) {
								if (app.getDataMode().equalsIgnoreCase("live"))
									modeValue = "live";
								else if (app.getDataMode().equalsIgnoreCase(
										"cache"))
									modeValue = "cache";
							} else
								modeValue = "live";
