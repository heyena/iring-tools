...
public class DataModel {
	...
	protected DataTransferIndices getDtis(String serviceUri,
		...
		try {
			...
			DataFilter dataFilter = dFI.AppendFilter(dataFilterUI,
					dataFilterFile);
			...
			if (dataFilter != null) {
				...
				} else // new filter or same filter after exchange, fetch
						// filtered data
				{
	...
	protected DataTransferIndices getFilteredDtis(DataFilter dataFilter,
		 ...
		 * ;
		 * 
		 * JaxbUtils.write(dataFilter,path, false); } catch (JAXBException e1) {
		...
		try {
			...
			if (transferTypeExpression != null && cloneFullDtiList != null) {
				String value = transferTypeExpression.getValues().getItems()
						.get(0);
					...
					Comparator<DataTransferIndex> comparator = new Comparator<DataTransferIndex>() {
						...
						/*	String dti1SortIndex = dti1.getSortIndex();
							...
							} else // sort type is numeric
							{
								...
								if (dti2SortIndex == null) {
									dti2SortIndex = String
											.valueOf(Double.MAX_VALUE);
								}

								if (sortDir.equals("asc")) {
									compareValue = (int) (Double
											.parseDouble(dti1SortIndex) - Double
											.parseDouble(dti2SortIndex));
								} else {
									compareValue = (int) (Double
	...
	protected DataTransferObjects getDtos(String serviceUri,
		...
		try {
			...
			dtos = httpClient.post(DataTransferObjects.class, dtoRelativePath,
					dxoRequest);
		} catch (HttpClientException e) {
			logger.error(e.getMessage());
			throw new DataModelException(e.getMessage());
