...
function showIndividualInfo(individual, classIdentifier, relatedClasses) {
	...
	for ( var colData in rowData) {
		...
		if (propertyName !== 'Content' && propertyName !== 'Info'
				&& propertyName !== 'Status')
	...
	var task = {
		run : function() {
			if (getProgressBoo) {
				Ext.Ajax
						.request({
							url : 'exchangeRequestStatus?scope=' + scope
									+ '&xid=' + xid,
							timeout : 5000,
							method : 'GET',
...
function deleteGraph(node, event) {
	...
	Ext.Msg.show({
		title : 'Delete Graph',
		msg : '<textarea ' + style + ' readonly="yes">'
				+ 'Are you sure you want to delete ' + name + ' graph?'
				+ '</textarea>',
		buttons : Ext.MessageBox.YESNO,
		modal : true,
		fn : function(buttonId, text) {
							...
							items : [
									...
									},
									{
										...
										listeners : {
											scope : this,
											select : function() {
												var appName = Ext.getCmp(
														'sourceAppCombolist')
														.getValue();
												var obj = Ext
														.getCmp('newExchConfig');
												var form = obj.getForm();
												var AppName = form.findField(
														'sourceAppName')
														.getValue();
												var context = form.findField(
														'sourceScopeName')
														.getValue();
												var Uri = form.findField(
														'sourceUri').getValue();
...
function SourceUri() {
	...
	var style = 'style="margin:0;padding:0;width:' + 400 + 'px;height:' + 160
			+ 'px;border:1px solid #aaa;overflow:auto"';
	Ext.Ajax.request({
		url : 'SourcetestUri?' + '&sourceUri =' + sourceUri + '&graphName ='
				+ graph,
		method : 'POST',
		timeout : 120000,
		success : function(response, request) {
...
function TargetUri() {
	...
	var style = 'style="margin:0;padding:0;width:' + 400 + 'px;height:' + 160
			+ 'px;border:1px solid #aaa;overflow:auto"';
	Ext.Ajax.request({
		url : 'testTargetUrl?' + '&targetUri =' + targetUri + '&graphName ='
				+ graph,
		method : 'POST',
		timeout : 120000,
		success : function(response, request) {
...
function saveExchangeConfig() {
	...
	if (((targetAppName !== "") && (targetAppName !== null))
			&& ((targetScopeName !== "") && (targetScopeName !== null))
			&& ((targetUri !== "") && (targetUri !== null))
			&& ((targetGraphName !== "") && (targetGraphName !== null))
			&& ((sourceAppName !== "") && (sourceAppName !== null))
			&& ((sourceScopeName !== "") && (sourceScopeName !== null))
			&& ((sourceUri !== "") && (sourceUri !== null))
			&& ((sourceGraphName !== "") && (sourceGraphName !== null))
			&& ((name !== "") && (name !== null))) {
		...
		if ((targetAppName === "")) {
			form1.findField('targetAppName').markInvalid(
					'App Name cannot be Empty');
		}

		if ((targetScopeName === "")) {
			form1.findField('targetScopeName').markInvalid(
					'Scope Name cannot be Empty');
		}
		if ((targetUri === "")) {
			form1.findField('targetUri').markInvalid(
					'Scope Uri cannot be Empty');
		}
		if ((targetGraphName === "")) {
			form1.findField('targetGraphName').markInvalid(
					'Graph Name cannot be Empty');
		}
		if ((sourceAppName === "")) {
			form1.findField('sourceAppName').markInvalid(
					'App Name cannot be Empty');
		}

		if ((sourceScopeName === "")) {
			form1.findField('sourceScopeName').markInvalid(
					'Scope Name cannot be Empty');
		}
		if ((sourceUri === "")) {
			form1.findField('sourceUri').markInvalid(
					'Scope Uri cannot be Empty');
		}
		if ((sourceGraphName === "")) {
			form1.findField('sourceGraphName').markInvalid(
					'Graph Name cannot be Empty');
		}
		if (name === "") {
...
function getAppMode() {
	...
	var obj = Ext.getCmp('directory-tree').getSelectionModel()
			.getSelectedNode();
	...
	Ext.Ajax.request({
		url : 'getAppDataMode?' + '&scope =' + context + '&name =' + AppName
				+ '&sourceUri =' + Uri,
		method : 'POST',
		timeout : 120000,
		success : function(response, request) {
		...
		failure : function(response, request) {
			Ext.Msg.show({
				title : 'Result ',
				msg : '<textarea ' + style + ' readonly="yes">'
						+ "Unable to get application info from endpoint."
						+ '</textarea>',
				buttons : Ext.MessageBox.OK
			});
		}
...
function fillShowUpdateCache() {
	...
	Ext.Ajax
			.request({
				url : 'getShowUpdateCache?' + '&scope =' + scope + '&appName ='
						+ appNameValue + '&name =' + graphValue + '&baseUri ='
						+ Uri,
				method : 'POST',
				timeout : 120000,
				success : function(response, request) {
					...
					if (showUpdatedinfo !== null) {
						...
						if (showUpdatedinfo.timeout == null
								|| showUpdatedinfo.timeout === 0) {
					...
					form.setValues({
						...
						displayTimeout : timeoutvalue+ " minutes"
					});
				...
				failure : function(response, request) {
					centerPanel.getEl().unmask();
					showDialog(400, 100, 'Loading Error',
							'Error getting cache information!', Ext.Msg.OK,
							null);
				}
			});
}
...
function editCommodity() {
	...
	Ext.Ajax.request({
		...
		failure : function(response, request) {
			centerPanel.getEl().unmask();
			showDialog(400, 100, 'Error',
					'Error getting commodity information!', Ext.Msg.OK, null);
		}
	});
}
	...
	Ext.Ajax.request({
		...
		success : function(response, request) {
			...
			graphNameStore = new Ext.data.ArrayStore({
				...
				url : 'getGraphNames?' + '&scope =' + context + '&name ='
						+ appNameValue + '&sourceUri =' + Uri,
				autoDestroy : true,
				autoLoad : false,
				fields : [ {
					name : 'value'
...
function newGraph() {
	...
	var graphNameStore = new Ext.data.ArrayStore({
		...
		url : 'getGraphNames?' + '&scope =' + context + '&name =' + AppName
				+ '&sourceUri =' + Uri,
		autoDestroy : true,
		autoLoad : false,
		fields : [ {
	...
	Ext.Ajax
			.request({
				...
				success : function(response, request) {
					 ...
					 * application.baseUri, oldAppName : appNameValue, oldScope :
					 * scope });
					 */
					var obj = Ext.getCmp('newAppForm');
					var form = obj.getForm();
					...
					AppNameStore = new Ext.data.ArrayStore({
						...
						url : 'getAppNames?' + '&appScope ='
								+ application.context + '&sourceUri ='
								+ application.baseUri,
						autoDestroy : true,
						autoLoad : false,
						fields : [ {
					...
					form.findField('oldAppName').setValue(
							application.displayName);
				...
				failure : function(response, request) {
					centerPanel.getEl().unmask();
					showDialog(400, 100, 'Error',
							'Error getting application information!',
							Ext.Msg.OK, null);
				}
			});
}
...
function editDeleteScope() {
	...
	Ext.Ajax.request({
		...
		failure : function(response, request) {
			centerPanel.getEl().unmask();
			showDialog(400, 100, 'Error', 'Error getting scope information!',
					Ext.Msg.OK, null);
		}
	});
}
...
function saveComm(node, button, event) {
	...
	Ext.Ajax
			.request({
				...
				failure : function(response, request) {
					showDialog(400, 100, 'Error', 'Error saving commodity!',
							Ext.Msg.OK, null);
				}
			});
}
		...
		success : function(response, request) {
			...
			if (result.success === true) {
				...
				if (name === "app") {
					...
					Ext.getCmp('sourceScopeCombolist')
							.bindStore(ScopeNameStore);
					Ext.getCmp('sourceScopeCombolist').setDisabled(false);
				} else if (name === "TargetExchange") {
					Ext.getCmp('targetScopeCombolist')
							.bindStore(ScopeNameStore);
							...
							listeners : {
								...
								render : function(c) {
									c.getEl().on(
											'keyup',
											function() {
												Ext.getCmp('scopeCombolist')
														.clearValue();
												Ext.getCmp('scopeCombolist')
														.setDisabled(true);
												Ext.getCmp('appCombolist')
														.clearValue();
												Ext.getCmp('appCombolist')
														.setDisabled(true);
											}, c);
						 ...
						 */{
							...
							listeners : {
								...
								select : function() {
									...
									var obj = Ext.getCmp('newAppForm');
									var form = obj.getForm();
									form.findField('displayName').setDisabled(
											false);
	...
	Ext.Ajax
			.request({
				...
				failure : function(response, request) {
					centerPanel.getEl().unmask();
					showDialog(400, 100, 'Error',
							'Error getting existing data filter!', Ext.Msg.OK,
							null);
				}
			});
	Ext.Ajax.request({
		...
		failure : function(response, request) {
			centerPanel.getEl().unmask();
			showDialog(400, 100, 'Error',
					'Error getting existing data filter!', Ext.Msg.OK, null);
		}
	});
	if (dfList !== null) {
		centerPanel.getEl().unmask();
...
function saveDataFilter() {
	...
	Ext.Ajax.request({
		...
		failure : function(response, request) {
			showDialog(400, 100, 'Error', 'Error saving data filter!',
					Ext.Msg.OK, null);
		}
	});
}
