...
if %dbUser% equ "" if %dbPassword% equ "" (@echo Windows authentication SQLCMD -S %dbServer% -i  Scripts\sandbox.sql) 
if %dbUser% neq "" if %dbPassword% neq "" (@echo SqlServer authentication SQLCMD -U %dbUser% -P %dbPassword% -S %dbServer% -i  .\Scripts\sandbox.sql)
...
set new_key="    <add key=\"/SandboxService/sparql\" value=\"noreuse,rdfs+sqlserver:rdf:Database=rdf;data source=%dbServer%; Initial Catalog=%dbCatalog%; User Id=%dbUser%; Password=%dbPassword%;\"/>"
