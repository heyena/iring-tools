...
{
    public class RestDataLayer3 : BaseDataLayer
    {
        ...
        private ILog _logger = LogManager.GetLogger(typeof(RestDataLayer3));

        private Dictionary<string, string> _objectyDictionary = null;
        //   private IDictionary<string, string> _selfUrl = null;

        public RestDataLayer3(AdapterSettings settings)
            : base(settings)
        {
        ...
        //  NP
        public IList<IDataObject> GetNew(string objectType, IList<string> Object_Names, IList<string> identifiers_Values)
        {
            try
            {
                ...
                url = GenerateUrl(objectType);

                for (int iCntr = 0; iCntr < Object_Names.Count; iCntr++)
                {
                    if (url.Contains(Object_Names[iCntr]))
                    {
                        if (!String.IsNullOrEmpty(identifiers_Values[iCntr]))
                            url = url.Replace(Object_Names[iCntr], Object_Names[iCntr] + "/" + identifiers_Values[iCntr]);
                    }
                }
        ...
        //  NP
        public override IList<IDataObject> Search(string objectType, string query, DataFilter filter=null, int pageSize=0, int startIndex=10)
        {
            string url = GetObjectUrl(objectType);
            string SearchURL = url + "/" + Constants.Search + "?" + Constants.SearchParamName;
            SearchURL  += "=" + query;

            string jsonString = GetJsonResponseFrom(SearchURL);
