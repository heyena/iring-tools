...
.\Bin\rdfstorage.exe -out "sqlserver:rdf:Database=rdf;data source=%dbServer%; Initial Catalog=sandbox; Integrated Security=SSPI;"

rem Update web.config file
@echo Updating web configuration file ...
set old_key=".*key=\"/SandboxService/sparql\".*"
set new_key="    <add key=\"/SandboxService/sparql\" value=\"noreuse,rdfs+sqlserver:rdf:Database=rdf;data source=%dbServer%; Initial Catalog=sandbox; Integrated Security=SSPI;\"/>"
