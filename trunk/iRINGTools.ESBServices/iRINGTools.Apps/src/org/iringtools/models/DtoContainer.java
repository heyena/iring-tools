...
public class DtoContainer
{
  ...
  {
    if (roObj.getOldValue() != null && !roObj.getOldValue().equals(roObj.getValue()))
    {
      setRValue("<span style=\"color:blue\">" + roObj.getOldValue() + " -> " + roObj.getValue() + "</span>");
    }
    else
    {
  ...
  {
    /*
     * if (rType.equals("OBJECT_PROPERTY")) { if (dunit == null) { dunit = new HashMap<String, String>();
     * retriveRValue(rval.substring(4)); this.rValue = tempName; } else { if (dunit.containsKey(rval)) this.rValue =
     * dunit.get(rval); else { retriveRValue(rval); this.rValue = tempName; }
     * 
     * } }
     */
    this.rValue = rval;
