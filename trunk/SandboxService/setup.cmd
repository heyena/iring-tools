...
if %dbUser% equ "" if %dbPassword% equ "" (
  SQLCMD -S %dbInstance% -i  .\Scripts\sandbox.sql  
  if %errorlevel% equ 0 (
    goto InitRDF
  )  
)

SQLCMD -U %dbUser% -P %dbPassword% -S %dbInstance% -i  .\Scripts\sandbox.sql
if %errorlevel% equ 0 (
  goto InitRDF
) 
...
.\Bin\rdfstorage.exe -out "sqlserver:rdf:Database=rdf;data source=%dbInstance%; Initial Catalog=sandbox; User Id=sandbox; Password=sandbox;"

rem Update web.config file
@echo Updating web configuration file ...
set old_key=".*key=\"/SandboxService/sparql\".*"
set new_key="    <add key=\"/SandboxService/sparql\" value=\"noreuse,rdfs+sqlserver:rdf:Database=rdf;data source=%dbInstance%; Initial Catalog=sandbox; User Id=sandbox; Password=sandbox;\"/>"
