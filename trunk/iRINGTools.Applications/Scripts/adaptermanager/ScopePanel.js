...
*/
Ext.define('AdapterManager.ScopePanel', {
  extend: 'Ext.panel.Panel',
  alias: 'widget.AdapterManager.ScopePanel',
  layout: 'fit',
  border: false,
  frame: false,
  split: true,
  ...
  */
  ...
  checkidNodeExists: function () {
    var returnVal = true
    for (var i = 0; i < Ext.getCmp('Directory-Panel').getStore().getRootNode().childNodes.length; i++) {
      if (Ext.getCmp('Directory-Panel').getStore().getRootNode().childNodes[i].text == this.form.getForm().getFieldValues().Name) {
        returnVal = false
      }
    }
    return returnVal
