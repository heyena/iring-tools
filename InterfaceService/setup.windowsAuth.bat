...
SQLCMD -U %dbUser% -P %dbPassword% -S %dbServer% -i  .\Scripts\iRING.sql
if %errorlevel% equ 0 (
  goto InitRDF
) 
...
.\Bin\rdfstorage.exe -out "sqlserver:rdf:Database=rdf;data source=%dbServer%; Initial Catalog=iring;Integrated Security=SSPI;"

rem Update web.config file
@echo Updating web configuration file ...
set old_key=".*key=\"/InterfaceService/sparql\".*"
set new_key="    <add key=\"/InterfaceService/sparql\" value=\"noreuse,rdfs+sqlserver:rdf:Database=rdf;data source=%dbServer%; Initial Catalog=iring; Integrated Security=SSPI;\"/>"
